...
Historical record. Dates are April to May 2026.

## Version Map

| Version | Architecture | Decode t/s | Status |
|---------|-------------|------------|--------|
| v2 | Monolithic, Q4_K/Q5_K/Q6_K, sigmoid scan | n/a | never coherent |
| v3 | Hybrid arch discovered, DN still elementwise | n/a | never coherent |
| v4 | DN state matrix + delta rule | n/a | never coherent |
| v5 | F32 GGUF, softmax replaces sigmoid scan | n/a | never coherent |
...
| v13_exp_layout | x4 layout | n/a | correct but 18% slower, abandoned |
| v13_plus_phase2 | x4 phase 2 | n/a | garbage output, abandoned |
...
v10:  6.98 t/s   <- transpiler rewrite + f32xf32 GEMV
v11:  4.1-4.6    <- merged optimizations (sustained; early 8.46 was 4 tokens only)
v12:  11.7 t/s   <- W8A8 SignedDot kernel (+2.5x over v11)
...
### Phase 1: Getting to Coherent Output (Apr 28 to May 1)
...
| Apr 29 19:22 | Output garbled, 7 suspected causes identified | All non-stretch items implemented | Fix 6 (sigmoid to softmax) later confirmed; Fix 7 (outer product swap) found later |
| Apr 29 to 30 | 7 bugs fixed across 4 sessions | conv1d, sigmoid underflow, Q6_K, signed byte, ssm_a, softmax, outer product swap | All valid fixes |
...
### Phase 2: Performance Optimization (May 1 to May 6)
...
| May 2 | 4 failed trials: pmaddubs, pmovsxbd, exp_approx, RoPE precompute | All failed for various reasons | exp_approx: 1.4% error caused early EOS |
...
| May 3 22:45 | v10: swap i8×i8 to f32×f32 GEMV | **6.98 t/s, CORRECT** | v8 `comptime for` pattern was correct all along: misdiagnosed |
