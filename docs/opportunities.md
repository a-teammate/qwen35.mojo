...
|---|-------------|----------------|-------|
| O1 | **Fix `run_tiles` correctness** | +23–30% decode | 8 lines of code. The speedup was measured but output was wrong. Root cause: likely Mojo closure capture or atomic ordering in `fetch_add` work-stealing. Debug: single-thread for-loop test → `parallelize` with 1 thread → `AtomicFence()` after quantize → static partitioning. |
...
| O4 | **Persistent thread pool** | +5–10% | Pin threads to physical cores with `sched_setaffinity`. Eliminate 15K→90 context switches. Requires FFI to POSIX. |
